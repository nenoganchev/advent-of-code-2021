...
public:
    ...
    snailfish_number operator + (const snailfish_number &rhs) const;
...
int main(int argc, char *argv[]) {
    ...
    cout << "Searching for largest sum... ";
    uint64_t largest_magnitude = 0;
    for (size_t i = 0; i < numbers.size(); ++i) {
        for (size_t j = i + 1; j < numbers.size(); ++j) {
            uint64_t sum1_magnitude = (numbers[i] + numbers[j]).magnitude();
            uint64_t sum2_magnitude = (numbers[j] + numbers[i]).magnitude();

            if (sum1_magnitude > largest_magnitude) largest_magnitude = sum1_magnitude;
            if (sum2_magnitude > largest_magnitude) largest_magnitude = sum2_magnitude;
        }
    }
    cout << largest_magnitude << endl;
}
...
snailfish_number snailfish_number::operator + (const snailfish_number &rhs) const {
    snailfish_number result = *this;
    result += rhs;
    return result;
}
