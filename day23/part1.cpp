...
state::state(const string &filename): _energy(0) {
    ...
    input_file >> expect("##\n")
               >> expect("  #");
    for (location room : {roomA, roomB, roomC, roomD}) {
        process_tile(room, 1, input_file.get());
        input_file >> expect("#");
    }
    input_file >> expect("\n");
    expect_line(input_file, "  #########");
...
void state::each_reachable_location(const amphipod &pod, const std::function<void(location, int, int)> &block) const {
    ...
    if (!occupied(pod.destination, 0) && !occupied(pod.destination, 1)) {
        block(pod.destination, 1, distance_to_destination_entry_pos + 2);
    }
    else if (!occupied(pod.destination, 0)) {
        block(pod.destination, 0, distance_to_destination_entry_pos + 1);
    }
}
...
bool state::amphipod::has_free_access_to_hallway() const {
    ...
    return position == 0 || !p_state->occupied(location, 0);
}
...
bool state::amphipod::has_free_access_to_destination() const {
    ...
    if (location != hallway) {
        if (position == 1 && p_state->occupied(location, 0)) return false;
